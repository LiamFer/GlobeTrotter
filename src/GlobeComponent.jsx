...
export default function GlobeComponent({ onCountrySelect }) {
  ...
  const [hoverD, setHoverD] = useState(); // <- país em hover
  ...
  return (
    <div style={{ height: '100vh', width: '100%' }}>
      <Globe
        ref={globeRef}
        backgroundColor="rgb(11, 11, 27)" // <- COR DO FUNDO
        globeImageUrl="https://unpkg.com/three-globe@2.42.3/example/img/earth-day.jpg"
        polygonsData={countries}
        polygonAltitude={({ properties: d }) => (d === hoverD ? 0.06 : 0.01)}
        polygonCapColor={({ properties: d }) =>
          d === hoverD ? 'rgba(0, 255, 149, 0.46)' : 'rgba(200, 200, 255, 0)'
        }
